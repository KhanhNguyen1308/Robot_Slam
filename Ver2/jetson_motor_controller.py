...
class JetsonMotorController:
    ...
    def __init__(self, 
                 ...
                 left_ms1_pin: int = 29,  # Optional MS1
                 left_ms2_pin: int = 31,  # Optional MS2
                 left_ms3_pin: int = 11,  # Optional MS3
                 right_step_pin: int = 32,  # Pin 32 (GPIO 12)
                 right_dir_pin: int = 36,   # Pin 36 (GPIO 16)
                 right_enable_pin: int = 38, # Pin 38 (GPIO 20)
                 right_ms1_pin: int = 13, # Optional MS1
                 right_ms2_pin: int = 15, # Optional MS2
                 right_ms3_pin: int = 16, # Optional MS3
                 wheel_diameter: float = 0.066,  # 66mm wheels
                 wheel_base: float = 0.275,      # 275mm between wheels
                 steps_per_rev: int = 200,
                 microsteps: int = 16,
                 gear_ratio: float = None):  # Motor rotations per wheel rotation
        ...
        # Motor must spin gear_ratio times for wheel to complete 1 revolution
        self.steps_per_meter = (self.steps_per_rev * self.gear_ratio) / self.wheel_circumference
        ...
        logger.info(f"Gear ratio: {self.gear_ratio:.3f}:1 (Motor:Wheel)")
    ...
    def set_microstepping(self, microsteps: int) -> bool:
        ...
        try:
            ...
            # Recalculate steps per meter with gear ratio
